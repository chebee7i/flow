...
void DynamicSolverTool::mainButtonReleased(const ToolBox::ButtonReleaseEvent & buttonReleaseEvent)
{
   ...
   if (data.cluster_size > 1)
   {
      ...
      {
         float x=pos[0] + (float) rand() / (float) RAND_MAX * 0.1 - 0.05;
         float y=pos[1] + (float) rand() / (float) RAND_MAX * 0.1 - 0.05;
         float z=pos[2] + (float) rand() / (float) RAND_MAX * 0.1 - 0.05;

         DynamicSolverData::PointArray a;

         for (unsigned int j=0; j < data.history_size; j++)
            a.push_back(Vrui::Point(x, y, z));
...
void DynamicSolverTool::drawBasicLines() const
{
   ...
   if (data.colorStyle == DynamicSolverData::SOLID)
   {
      ...
      for (unsigned int i=0; i < data.points.size(); i++)
      {
         ...
         {
            glVertex3f(data.points[i][j - 1][0], data.points[i][j - 1][1], data.points[i][j - 1][2]);
            glVertex3f(data.points[i][j][0], data.points[i][j][1], data.points[i][j][2]);
         }
         glEnd();
      }
   }
...
void DynamicSolverTool::drawPolylines() const
{
   ...
   for (unsigned int i=0; i < data.points.size(); i++)
   {
      ...
      {
         // set up gle data
         pts[j][0]=data.points[i][j][0];
         pts[j][1]=data.points[i][j][1];
         pts[j][2]=data.points[i][j][2];
      }
...
void DynamicSolverTool::drawPointHeads(DTS::DataItem* dataItem) const
{
   ...
   glBegin(GL_POINTS);
   for (unsigned int i=0; i < data.points.size(); i++)
      glVertex3f(data.points[i][0][0], data.points[i][0][1], data.points[i][0][2]);
...
void DynamicSolverTool::drawSphereHeads() const
{
   ...
   {
      glPushMatrix();
      glTranslatef(data.points[i][0][0], data.points[i][0][1], data.points[i][0][2]);
