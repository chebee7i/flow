...
class StaticSolverData
{
   ...
   private:
      ...
      std::vector<Vrui::Point> points; ///< Actual point data.
...
class StaticSolverTool: public AbstractDynamicsTool, public GLObject
{
   ...
   public:
      ...
      virtual void setIntegrator(Integrator* i);
      virtual void updatedIntegrator();
      virtual void step();
         ...
         if (size > datasets[0]->numberOfPoints)
         {
            ...
            for (it = datasets.begin(); it != datasets.end(); it++)
            {
               ...
               for (unsigned int i=data->numberOfPoints; i < size; i++)
               {
                  ...
                  data->points[i]+=integrator->step(data->points[i - 1]);
               }
            }
         }
