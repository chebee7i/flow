...
void ParticleSprayerTool::step()
{
   ...
   Vrui::Point old; // previous position of particle
   float velocity;
   ...
   for (Data::ParticleArray::iterator particle=data.particles.begin(); particle
         != data.particles.end(); ++particle)
   {
      ...
      (*particle).pos+=integrator->step((*particle).pos);

      // compute the velocity of the particle
      velocity=(((*particle).pos[0] - old[0]) * ((*particle).pos[0] - old[0])
            + ((*particle).pos[1] - old[1]) * ((*particle).pos[1] - old[1])
            + ((*particle).pos[2] - old[2]) * ((*particle).pos[2] - old[2]));

      if (check_max)
      {
         next_max=(velocity > next_max ? velocity : next_max);
      }

      int index=(int) (sqrt(velocity) / sqrt(max_vel) * 255.0);
